defmodule Web3MudExExample.World.Loader do
  @moduledoc """
  ...
  alias Web3MudExExample.World.Zone
  ...
  def load(paths \\ %{}) do
    ...
    brain_data = Web3MudExExample.Brain.load_all(paths.brains_path)
  ...
  def parse_character(zone, key, character_data, brains) do
    character = %Character{
      ...
      brain: Web3MudExExample.Brain.process(Map.get(character_data, :brain), brains),
      meta: %Web3MudExExample.Character.NonPlayerMeta{
        zone_id: zone.id,
  ...
  defp parse_initial_events(%{initial_events: initial_events}) do
    Enum.map(initial_events, fn initial_event ->
      %Web3MudExExample.Character.InitialEvent{
  ...
  def parse_item(zone, key, item_data, verbs) do
    ...
    item = %Item{
      ...
      verbs: item_verbs,
      callback_module: Web3MudExExample.World.Item,
  ...
  def generate_minimap(zone) do
    mini_map = %Web3MudExExample.MiniMap{id: zone.id}

    cells =
      Enum.map(zone.rooms, fn room ->
        %Web3MudExExample.MiniMap.Cell{
          id: room.id,
          ...
          y: room.y,
          z: room.z,
          connections: %Web3MudExExample.MiniMap.Connections{
